...
| 2 | Native AI blocker: `ContentGuardAgent` (capture + NudeNet) + `ContentGuardDaemon` (tamper anchor) | **in progress** - signing test done (self-signed cert confirmed working for PPPC, see `docs/PHASE_2_SIGNING_TEST.md`); `Shared/`, `ContentGuardAgent/Sources/`, `ContentGuardDaemon/Sources/`, `LaunchAgents/`, `LaunchDaemons/`, and `Installer/` all written. Still open: create `ContentGuard.xcodeproj` in Xcode on the real Mac, add the real ONNX Runtime SPM dependency and confirm it against `NudeNetClassifier.swift`'s TODOs, place the real model at `Model/nudenet_640m.onnx`, verify `EscalationManager.classifyExit(pid:)`'s exit-status mechanism for real, then run the full Phase 2 verification checklist |
