...
| 2 | Native AI blocker: `ContentGuardAgent` (capture + NudeNet) + `ContentGuardDaemon` (tamper anchor) | **in progress** - signing test done (self-signed cert confirmed working for PPPC, see `docs/PHASE_2_SIGNING_TEST.md`); `ContentGuard.xcodeproj` created on the real Mac with all three targets (`ContentGuardAgent`, `ContentGuardDaemon`, `ContentGuardRelease`) building clean, a handful of real compile bugs (missing selector method, Swift/libc `connect` name collision, a type-checker timeout) found and fixed along the way. Still open: add the real ONNX Runtime SPM dependency and confirm it against `NudeNetClassifier.swift`'s TODOs, place the real model at `Model/nudenet_640m.onnx`, verify `EscalationManager.classifyExit(pid:)`'s exit-status mechanism for real, then run the full Phase 2 verification checklist |
