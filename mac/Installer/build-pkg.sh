...
echo ""
echo "    Do NOT install ContentGuard-$VERSION.pkg directly - confirmed live on the"
echo "    real Mac (macOS 26.5): sudo installer refuses it with 'This package"
echo "    requires Rosetta 2 to be installed', even though every binary in this"
echo "    build is 100% native arm64 (verified with \`file\` on every Mach-O in"
echo "    Installer/pkg-root, and grepped every PackageInfo/Distribution for any"
echo "    architecture restriction - there is none). The Rosetta check is"
echo "    specific to productbuild's product-archive wrapper on this macOS"
echo "    version, not anything this build actually needs at runtime."
echo ""
echo "    Install the three components directly instead - confirmed working,"
echo "    no Rosetta involved:"
echo "      sudo installer -pkg Installer/contentguard-app.pkg -target /"
echo "      sudo installer -pkg Installer/daemon-plist.pkg -target /"
echo "      sudo installer -pkg Installer/agent-plist.pkg -target /"
