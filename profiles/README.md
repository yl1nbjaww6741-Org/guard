...
|---|---|---|
| `restrictions.mobileconfig` | Ready to push (v6) | `forceAdminPasswordForAppInstallation` deliberately set `false`, not the original spec's `true` - see below. Also see the flag below about `allowScreenshotsAndScreenRecording` before Phase 2. As of v4 (2026-08-25), also blocks every browser except the hardened Chrome stable channel via `blacklistedAppBundleIDs` (explicit user request); v5 (same day) deepened that list and added torrent/P2P clients to it - see the profile's own comment for the reasoning, confidence tiers, and its stated limitation. v6 (2026-09-25) sets `allowAccountModification` to `true` (explicit user request) - `false` locked the whole Apple Account pane, iCloud settings included, and there's no narrower key; see the profile's comment for what that gives up. |
