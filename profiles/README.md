...
|---|---|---|
| `restrictions.mobileconfig` | Ready to push (v4) | `forceAdminPasswordForAppInstallation` deliberately set `false`, not the original spec's `true` - see below. Also see the flag below about `allowScreenshotsAndScreenRecording` before Phase 2. As of v4 (2026-08-25), also blocks every browser except the hardened Chrome stable channel via `blacklistedAppBundleIDs` (explicit user request) - see the profile's own comment for the reasoning and its stated limitation. |
